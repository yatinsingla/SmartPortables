...
public class OrderProcessSalesmen extends HttpServlet implements Serializable {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MySQLDataStoreUtilities database;
                            ...
                            out.println("<ul class=\"products\">");
                                ...
                                out.println("<form>");
                                    ...
                                    if(process.equals("update")){
                                        int result = 0;
                                        database = new MySQLDataStoreUtilities();
                                        result = database.orderUpdate(orderNum);
                                        if(result != 0 ){
                                            out.println("<tr><td>The Order Number "+orderNum+" has been updated by changing the status = 2</td></tr>");
                                            out.println("<tr><td><a href='ViewOrderSalesmen'>View Change</a></td></tr>");
                                        }else{
                                            out.println("<tr><td>The Order Number has not been updated</td></tr>");
                                        }	
                                    }
                                    ...
                                    if(process.equals("delete")){	
                                        int result = 0;
                                        database = new MySQLDataStoreUtilities();
                                        result = database.orderCancel(orderNum);
                                        if(result != 0){
                                            out.println("<tr><td>The Order Number "+orderNum+"has been deleted</td></tr>");
                                            response.sendRedirect("ViewOrderSalesmen");
                                        }else{
                                            out.println("<tr><td>The Order Number has not been deleted</td></tr>");
                                        }
                                    }
