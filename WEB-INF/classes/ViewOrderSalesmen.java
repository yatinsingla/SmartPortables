...
public class ViewOrderSalesmen extends HttpServlet implements Serializable {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		hashObj = database.getOrderForSalesmen();
            ...
            out.println("<div id=\"container\">");
                ...
                out.println("<div id=\"body\">");
	                out.println("<section id=\"content\">");
                    	out.println("<article>");
                        out.println("<h3>View All Orders</h3>");
                        out.println("<a href='Salesmen'>Return to Main Menu</a>");
                            out.println("<ul class=\"products\">");

                                out.println("<form>");
                                out.println("<table border=1>");
                                for(Map.Entry<String, ArrayList<Orderbean>> map : hashObj.entrySet()){
                                    out.println("<tr><td><label>Order Number</label></td><td><label>Product Id</label></td><td><label>Product Name</label></td><td><label>Ordered By</label></td><td><label>Status</label></td><td></td><td></td></tr>");
                                    for(Orderbean userorder : map.getValue()){
                                        String status="";
                                        if(Integer.parseInt(userorder.getStatus()) ==1){
                                            status ="To be Delivered";
                                        }else{
                                            status ="Delivered";
                                        }
                                        String updateLink = "<a href='OrderProcessSalesmen?process=update&orderNumber="+userorder.getOrderNumber()+"'>Update</a>";
                                        String deleteLink = "<a href='OrderProcessSalesmen?process=delete&orderNumber="+userorder.getOrderNumber()+"'>Delete</a>";
                                        
                                        out.println("<tr><td>"+userorder.getOrderNumber()+"</td><td>"+userorder.getProductId()+"</td><td>"+userorder.getProductName()+"</td><td>"+userorder.getUserName()+"</td><td>"+status+"</td><td>"+updateLink+"</td><td>"+deleteLink+"</td></tr>");
